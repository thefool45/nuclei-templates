...
import os, sys, yaml, pathlib, requests, time, random
...
def notify_discord(payload):
	lst_webhook = [
		...
		os.getenv("DISCORD_WEBHOOK_FLYSEC")
	]
...
def get_shodan_query_count(query):
	...
	'''
	all_total = -1
	SHODAN_API = random.choice(os.getenv("SHODAN_KEY").split("\n"))
...
def main(action, template_path):
	...
	if shodan_query:
		if isinstance(shodan_query, str):
			shodan_query = [shodan_query]

		new_query = []
		for query in shodan_query:
			query = query.strip()
			total_query = get_shodan_query_count(query=query)
			new_query.append(f"{query} - {total_query}")

		new_query = "`\n- `".join(new_query)
		fields.append({"name": "Shodan", "value": f"- `{new_query}`", "inline": False})
